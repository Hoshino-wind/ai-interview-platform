...
export class QuestionGeneratorService {
  ...
  async generateQuestions(applicationId: string): Promise<GeneratedQuestion[]> {
    // 1. Get application with candidate and job info
    ...
    // 4. Generate questions with LLM or fallback to mock
    if (this.llmService.isAvailable()) {
      try {
        return await this.generateWithLLM(parsedData, job);
      } catch (error) {
        this.logger.warn(
          `LLM generation failed, falling back to mock: ${error instanceof Error ? error.message : String(error)}`,
        );
...
[{
  ...
  private parseGeneratedQuestions(response: string): GeneratedQuestion[] {
    try {
      ...
      return questions.map((q) => ({
        ...q,
        difficulty: (q.difficulty?.toUpperCase() || 'MEDIUM') as
          | 'EASY'
          | 'MEDIUM'
          | 'HARD',
      }));
  ...
  private generateMockQuestions(
    ...
    // Question 1: Business scenario question based on tech stack
    const frontendSkills = skills.filter((s) =>
      ['React', 'Vue', 'Angular', 'Next.js', 'TypeScript', 'JavaScript'].includes(
        s,
      ),
    );
...
您可以使用任何语言和工具。`,
    ...
    // Question 2: Debug/Refactor question (general)
    questions.push({
      title: '代码重构挑战',
...
鼓励使用任何工具辅助分析和重构。`,
    ...
    // Question 3: Based on job requirements (if specific domain)
...
您可以自由使用任何工具、框架和资源。`,
        difficulty:
          level === 'expert' ? 'HARD' : level === 'senior' ? 'MEDIUM' : 'EASY',
        estimatedTime: 50,
  ...
  /**
   * Map difficulty string to Prisma Difficulty enum
   */
  ...
  /**
   * Determine question type based on title and description
   */
  determineQuestionType(title: string, description: string): QuestionType {
    ...
    if (
      lowerTitle.includes('重构') ||
      lowerTitle.includes('refactor') ||
      lowerDesc.includes('重构')
    ) {
      return QuestionType.REFACTOR;
    }
    if (
      lowerTitle.includes('调试') ||
