...
export class MockScoringService {
  ...
  evaluateCodeQualityNew(code: string): CodeQualityDimension {
    ...
    // 可读性评分 (0-5)
    let readability = 3;
    if (comments > 0) readability += 0.5;
    if (lines < 50) readability += 0.5;
    if (functions > 0 && functions <= 5) readability += 0.5;
    if (hasTypes) readability += 0.5;
    ...
    if (camelCase > 0) naming += 1;
    if (pascalCase > 0) naming += 1;
    naming = Math.min(5, naming);

    // 代码结构评分 (0-5)
    let structure = 3;
    if (functions > 1) structure += 0.5;
    if (functions > 3) structure += 0.5;
    if (code.includes('import') || code.includes('export')) structure += 0.5;
    if (lines < 100) structure += 0.5;
   ...
   * 新版问题解决过程评估 (20分)
   */
  evaluateProblemSolvingNew(
    code: string,
    eventAnalysis: CodingEventAnalysis,
  ): ProblemSolvingDimension {
    const { runCodeCount, codeChangeCount, thinkingPauses, debugTimeRatio, totalDuration } = eventAnalysis;

    // 思路条理性 (0-7)
    let thinkingClarity = 4;
    if (thinkingPauses > 2) thinkingClarity += 1; // 有思考停顿
    if (totalDuration > 300) thinkingClarity += 0.5; // 足够时间思考
    if (codeChangeCount > 5 && codeChangeCount < 50) thinkingClarity += 1; // 合理的修改次数
    thinkingClarity = Math.min(7, thinkingClarity);

    // 调试策略 (0-7)
    let debugStrategy = 3;
    if (runCodeCount > 2 && runCodeCount < 15) debugStrategy += 2; // 适度的测试
    if (debugTimeRatio > 10 && debugTimeRatio < 40) debugStrategy += 1; // 合理的调试时间
    if (runCodeCount > 0 && codeChangeCount / runCodeCount > 1) debugStrategy += 1; // 每次运行都有改进
    debugStrategy = Math.min(7, debugStrategy);

    // 迭代改进 (0-6)
    let iterativeImprovement = 3;
    if (codeChangeCount > 10) iterativeImprovement += 1;
    if (runCodeCount > 3) iterativeImprovement += 1;
    if (thinkingPauses > 1) iterativeImprovement += 1;
  ...
  evaluateEngineeringNew(code: string): EngineeringDimension {
    ...
    // 错误处理 (0-5)
    let errorHandling = 2;
    if (hasTryCatch) errorHandling += 2;
    if (hasValidation) errorHandling += 1;
    ...
    const hasEfficientLoop =
      !code.includes('for.*for') && !code.includes('while.*while');
    if (hasOptimization) performance += 1;
    if (hasEfficientLoop) performance += 1;
    ...
    const hasModules =
      code.includes('import') || code.includes('export') || code.includes('require');
    if (functions > 1) modularity += 1;
    if (functions > 3) modularity += 1;
    if (hasModules) modularity += 1;
  ...
  generateReportNew(
    ...
    // Generate suggested questions based on new dimensions
    if (scores.deliveryQuality < 15) {
      suggestedQuestions.push(
        '测试用例有部分未通过，请分析一下可能的原因以及你的调试思路？',
      );
    }
    if (scores.codeQuality < 10) {
      suggestedQuestions.push(
        '代码质量方面还有提升空间，请谈谈你对代码可读性和命名规范的理解？',
      );
    }
    if (scores.problemSolving < 12) {
      suggestedQuestions.push(
        '在解决这个问题的过程中，你是如何规划解题步骤的？有没有遇到卡住的地方？',
      );
    }
    if (scores.toolUsage > 10) {
      suggestedQuestions.push(
        '你在编码过程中使用了外部资源，能分享一下你是如何高效利用这些资源的吗？',
      );
    }
    if (scores.engineering < 10) {
      suggestedQuestions.push(
        '工程化思维方面，你如何看待错误处理和代码模块化？',
      );
    }
    if (scores.techMatch < 6) {
      suggestedQuestions.push(
        '岗位要求中有一些技术栈，你是否有相关经验或学习计划？',
      );
    }
